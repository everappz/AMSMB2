...
static NSData *_Nullable AMBuildDirWire(uint8_t infoClass, NSArray<AMSMB2FileInfo *> *entries, NSUInteger *ioCursor, uint32_t budget, BOOL singleEntry)
{
    ...
    while (i < entries.count) {
        ...
        if (infoClass == SMB2_FILE_NAMES_INFORMATION) {
            ...
            // FileId (file reference number): a STABLE, per-file identifier (the inode). A non-zero
            // value lets clients cache each entry by reference number; with 0, macOS cannot populate
            // its vnode cache from the listing and re-queries every file individually (pass after
            // pass). The delegate supplies the inode via AMSMB2FileInfo.fileIdentifier; it must be
            // stable across queries, never a per-listing index (that aliases every lookup to the
            // first entry).
            uint64_t fileId = info.fileIdentifier;
            switch (infoClass) {
